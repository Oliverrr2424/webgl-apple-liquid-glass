...
真实水滴的陡斜面会让一段连续的出射像素看到近似同一个背景位置，所以树枝、楼灯
或海岸的颜色会作为一整条色带被拉到水滴轮廓，而不是越靠边越糊。现在外圈会把每条
法线方向上的采样坐标钉在斜面内侧的一个锚点，再平滑过渡回正常折射；同时采用锚点
处的模糊半径，避免 `blurRim` 把这条色带重新卷积散掉。面板中的 `rimStretch` 控制
这段“颜色延伸区”占 bevel 的比例；设为 `0` 可退回原先只靠折射与模糊的效果。
